...
/// <summary>Процедурный гранж: мазок кисти с шумными краями, сухие «зубья» на концах,
/// мотлинг-фактура (SrcATop), брызги с чипсами, потёртости (DstOut-«выгрызание»).</summary>
public static class Grunge
{
    /// <summary>Контур мазка кисти: края дрожат многооктавным value-шумом,
    /// торцы (лево/право) рваные сильнее горизонтальных краёв.</summary>
    public static SKPath BrushStrokePath(SKRect rect, int seed, float edgeAmp, float endAmp)
    {
        ...
        for (var x = rect.Left; x <= rect.Right; x += step)
            points.Add(new SKPoint(x, rect.Top + Fbm(x / scale, seed) * edgeAmp));

        for (var y = rect.Top; y <= rect.Bottom; y += step)
            points.Add(new SKPoint(rect.Right + Fbm(y / (scale * 0.4f), seed + 7) * endAmp, y));

        for (var x = rect.Right; x >= rect.Left; x -= step)
            points.Add(new SKPoint(x, rect.Bottom + Fbm(x / scale, seed + 13) * edgeAmp));

        for (var y = rect.Bottom; y >= rect.Top; y -= step)
            points.Add(new SKPoint(rect.Left + Fbm(y / (scale * 0.4f), seed + 23) * endAmp, y));
    ...
    /// <summary>Сухие «зубья» кисти на торце мазка: горизонтальные штрихи наружу + выгрызы внутрь.
    /// Вызывать внутри SaveLayer мазка — выгрызы идут через DstOut.</summary>
    public static void BrushTeeth(SKCanvas canvas, float edgeX, float top, float bottom, int direction,
        SKColor color, Random rng, float maxLength)
    {
        ...
        for (var y = top; y < bottom;)
        {
            ...
            canvas.DrawRect(new SKRect(MathF.Min(edgeX - direction * 4, x2), y,
            ...
            if (rng.NextDouble() < 0.45)
            {
                ...
                canvas.DrawRect(new SKRect(MathF.Min(edgeX, biteX2), biteY,
    ...
    /// <summary>Мотлинг: полупрозрачные пятна ТОЛЬКО по уже нарисованному (SrcATop) —
    /// фактура потёртой краски на буквах/мазке. Вызывать внутри SaveLayer.</summary>
    ...
    /// <summary>Кластер брызг краски: облако точек убывающего радиуса + квадратные «чипсы».</summary>
    public static void Splatter(SKCanvas canvas, SKPoint center, SKColor color, Random rng, int count, float spread)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            {
                Chip(canvas, new SKPoint(center.X + dx, center.Y + dy), color,
    ...
    /// <summary>Квадратные «чипсы» вдоль отрезка — характерные осколки цифровых кистей.</summary>
    public static void ChipsAlongLine(SKCanvas canvas, SKPoint from, SKPoint to, SKColor color, Random rng,
        int count, float spread)
    {
        for (var i = 0; i < count; i++)
        {
            ...
            Chip(canvas, new SKPoint(x, y), color, 2f + (float)rng.NextDouble() * 4.5f, rng);
        }
    }

    /// <summary>Потёртости: точки и царапины «выгрызаются» из уже нарисованного через DstOut.
    /// Вызывать ВНУТРИ SaveLayer вокруг объекта — иначе выест и фон под ним.</summary>
    ...
    /// <summary>Многооктавный value-шум в [-1, 1]: низкие частоты дают крупные «лохмотья» края,
    /// высокие — мелкую рваность.</summary>
